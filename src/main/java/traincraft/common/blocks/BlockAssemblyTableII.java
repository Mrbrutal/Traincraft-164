...
public class BlockAssemblyTableII extends BlockContainer {
	...
	public BlockAssemblyTableII(int i, int j, Material material) {
		super(i, material);
	...
	public Icon getBlockTexture(IBlockAccess worldAccess, int i, int j, int k, int side) {
		if (((TileCrafterTierII) worldAccess.getBlockTileEntity(i, j, k)).getFacing() != null) {
			side = TileHelper.getOrientationFromSide(((TileCrafterTierII) worldAccess.getBlockTileEntity(i, j, k)).getFacing(), ForgeDirection.getOrientation(side)).ordinal();
		}
	...
	public void breakBlock(World world, int i, int j, int k, int par5, int par6) {
		...
		TileCrafterTierII tileentitytierII = (TileCrafterTierII) world.getBlockTileEntity(i, j, k);
	...
	public void onBlockPlacedBy(World world, int i, int j, int k, EntityLivingBase entityliving, ItemStack stack) {
		TileCrafterTierII te = (TileCrafterTierII) world.getBlockTileEntity(i, j, k);
		if (te != null) {
