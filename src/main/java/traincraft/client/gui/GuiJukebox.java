		...
		if (button.id == 0) {
			if (streamTextBox.getText() != null && streamTextBox.getText().length() > 0) {
				if ((!jukebox.isPlaying())) {
					...
					PacketHandler.setJukeBoxStreamingUrl(this.player, this.jukebox, this.jukebox.streamURL, true);
					jukebox.startStream();
				}
				else {
					PacketHandler.setJukeBoxStreamingUrl(this.player, this.jukebox, this.jukebox.streamURL, false);
					jukebox.stopStream();
				}
			}
				...
				if ((!((AbstractTrains) jukebox).locked)) {
					...
					if (lis3 != null && lis3.size() > 0) {
						for (int j1 = 0; j1 < lis3.size(); j1++) {
							...
							if (entity instanceof EntityPlayer) {
								((AbstractTrains) jukebox).sendTrainLockedPacket((EntityPlayer) entity, true);
							}
						}
					}
					...
					if (lis3 != null && lis3.size() > 0) {
						for (int j1 = 0; j1 < lis3.size(); j1++) {
							Entity entity = (Entity) lis3.get(j1);
							if (entity instanceof EntityPlayer) {
								((AbstractTrains) jukebox).sendTrainLockedPacket((EntityPlayer) entity, false);
							}
						}
					}
